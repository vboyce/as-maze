...
```{r}
with_labels |> group_by(workerid) |> summarize(m=mean(correct)) |> ggplot(aes(x=m))+geom_histogram()+geom_vline(xintercept=.9, color="red")+labs(x="Mean Maze accuracy")
...
```{r}
include_error <- with_labels |> group_by(workerid) |> summarize(m=mean(correct)) |> filter(m>.9)
#88 people included
...
filtered <- with_labels |> inner_join(include_error |> select(workerid)) |> filter(rt>200) |> filter(rt<3000)
```
...
```{r}
filtered |> filter(correct==1)|> ggplot(aes(x=reorder(workerid, rt), y=rt))+geom_point(alpha=.005)+stat_summary(fun.data="mean_cl_boot")+theme(axis.ticks.x=element_blank(), axis.text.x=element_blank())+labs(x="Participant")+scale_y_continuous(breaks=seq(500,2000,250))
...
```{r}
filtered |> group_by(workerid, trial_index) |> summarize(m=mean(rt)) |>  ggplot(aes(x=trial_index, y=m, group=workerid))+geom_smooth(se=F)+labs(y="RT", x="Trial")
```
...
context_condition <- filtered |> ungroup() |>  select(workerid, type, number) |> unique() |> filter(type %in% c("1-context", "2-context")) |> 
  rename(context=type)

critical_only <- filtered |> filter(type %in% c("NP", "VP")) |> 
...
```{r}
critical_only |> group_by(workerid) |> summarize(m=mean(correct)) |> ggplot(aes(m))+geom_histogram()
```
...
filtered |> filter(type %in% c("NP", "VP")) |> 
  ...
  #filter(number!=1) |> 
  left_join(context_condition) |> ggplot(aes(x=type, y=rt, color=context))+  stat_summary(fun.data="mean_cl_boot", position=position_dodge(width=.7))+facet_wrap(~part)
...
```{r}
critical_all <- filtered |> mutate(split_critical=str_split(critical, " ")) |> 
  rowwise() |>
...
priors <- c(prior(normal(800,200), class="Intercept"),
...
grouped_mod <- for_mod |> filter(is_critical) |> group_by(item, participant, type.numeric, context.numeric) |> 
  summarize(correct=sum(correct), n=n(), rt=sum(rt)) |> filter(correct==n) |> mutate(multi=(n>1))

m_grouped <- brm(rt ~ type.numeric*context.numeric + multi+
            (type.numeric*context.numeric|item)+
...
Participants overall reading speed is pretty variable, so we could try to determine that on the basis of all the other words and add that as a predictor (rather than currently just estimating on the basis of the 16 critical regions). Alternatively, could add word-level properties in, etc. All of this would introduce more degrees of researcher freedom. 
